...
import ThreeDStationCanvas, { StationModelItem } from "./ThreeDStationCanvas";
...
const systems:Item[]=[
  ...
  {id:"comm",label:"Communications",type:"Comms",status:"normal",health:98,temperature:"−22°C",power:"18 kW",alert:"None",lastUpdate:"02s ago",x:-7,z:3.5,w:2.7,d:2.4,h:1.7},
...
export default function DigitalTwin({station}:{station:"MAITRI"|"BHARATI"}){
  ...
  return <div className="page-shell twin-page">
    <div className="page-heading">
      <div><div className="eyebrow">DIGITAL TWIN / {station}</div><h1 className="page-title">{station} DIGITAL TWIN</h1><p className="page-subtitle">Interactive 3D station model · drag to orbit · wheel to zoom · Shift/right-drag to pan · click a system to inspect</p></div>
    ...
    <div className="twin-layout">
      <section className="glass twin-viewport">
        <div className="viewport-hud"><span>3D DIGITAL TWIN</span><small>REAL-TIME PERSPECTIVE MODEL · MANUAL CAMERA</small></div>
        <ThreeDStationCanvas key={viewKey} items={model} selectedId={selectedId} onSelect={selectSystem} interactive command={cameraCommand} />
        <div className="viewport-legend"><span><i className="legend-dot normal"/> Operational</span><span><i className="legend-dot monitoring"/> Monitoring</span><span><i className="legend-dot warning"/> Attention</span></div>
      ...
      <aside className="glass twin-inspector">
        ...
        <div className="concept-note">Conceptual station representation for demonstration; geometry is schematic and is not an architectural claim about NCPOR facilities.</div>
      </aside>
    </div>
  </div>
}
